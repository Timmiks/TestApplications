...
//--------------------------------------------
void ofAppGLFWWindow::randomCrash() {
	if (startCrashTime < 0)
	{
		startCrashTime = ofGetCurrentTime().seconds;
	}

	int timeBetweenChecks = (ofGetCurrentTime().seconds - startCrashTime) % 10;
	int number = rand();

	if (number < RAND_MAX / 5 && timeBetweenChecks == 0)
	{
		throw std::logic_error("random 20% crash");
	}
}
